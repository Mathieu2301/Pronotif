  ...
  <div>
    ...
    <div class="pageContainer">
      ...
      <div class="block">
        ...
        <div class="content">
          <table v-if="data.timetable && data.timetable.length > 0">
            <tr>
          ...
          <div v-else>Aucun cours aujourd'hui</div>
        </div>
      </div>
      ...
      <div class="block">
        ...
        <div class="content">
          <table v-if="data.friends && data.friends.length > 0">
            <tr>
              <th class="left">Nom</th>
            </tr>

            <tr class="time"
              v-for="uname in data.friends"
              :key="uname"
            >
              <td class="left">{{ uname }}</td>
              <td class="right clickable" @click="removeFriend(uname)">❌</td>
            </tr>
          </table>
...
export default {
  ...
  methods: {
    ...
    removeFriend(uname) {
      if (!window[`${'confirm'}`](`Retirer "${uname}" de la liste ?`)) return;
      this.message.text = '';
      window.socket.emit('removeFriend', this.user, uname, (rs) => {
        if (rs.error) this.message = { type: 'error', text: rs.error };
        else this.message = { type: 'success', text: `Ami "${uname}" retiré !` };
      });
    },
